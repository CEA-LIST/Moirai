...
fn causal_delivery_twins() {
    ...
    let event_b_1 = replica_b.send(Counter::Inc(1));
    let event_b_2 = replica_b.send(Counter::Inc(1));
    let event_b_3 = replica_b.send(Counter::Inc(1));
    let event_b_4 = replica_b.send(Counter::Inc(1));
...
fn causal_delivery_triplet() {
    ...
    let event_b = replica_b.send(Counter::Inc(2));

    replica_a.receive(event_b.clone());
    let event_a = replica_a.send(Counter::Dec(7));
