...
//             i,
//             i + 1,
//             event.metadata.front().unwrap(),
//             event.op.replace("\"", ""),
//         ));
//         if process.contains_key(event.metadata.front().unwrap().origin()) {
//             let list = process
//                 .get_mut(event.metadata.front().unwrap().origin())
//                 .unwrap();
//             list.push(i);
//         } else {
//             process.insert(
//                 event.metadata.front().unwrap().origin().to_string(),
//                 vec![i],
//             );
//         }
...
//                     .unwrap()
//                     .view_id()
//                     .cmp(&event.metadata.front().unwrap().view_id())
//                 {
//                     Ordering::Less => {
//                         graph.push((j, i));
//                     }
