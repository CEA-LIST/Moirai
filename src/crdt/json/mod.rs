...
impl IsLog for JsonLogContainer {
    ...
    fn is_default(&self) -> bool {
        match &self.value {
            Some(JsonLog::Bool(log)) => log.is_default(),
            Some(JsonLog::Number(log)) => log.is_default(),
            Some(JsonLog::Object(log)) => log.is_default(),
            Some(JsonLog::String(log)) => log.is_default(),
            Some(JsonLog::Array(log)) => log.is_default(),
            None | Some(JsonLog::Null) => true,
        }
    }
}

impl EvalNested<Read<<Self as IsLog>::Value>> for JsonLogContainer {
    fn execute_query(
        &self,
        _q: Read<Self::Value>,
    ) -> <Read<Self::Value> as QueryOperation>::Response {
        match &self.value {
            Some(JsonLog::Bool(log)) => Value::Bool(log.execute_query(Read::new())),
            Some(JsonLog::Number(log)) => {
                Value::Number(serde_json::Number::from_f64(log.execute_query(Read::new())).unwrap())
            }
            Some(JsonLog::Object(log)) => {
            ...
            Some(JsonLog::Array(log)) => Value::Array(log.execute_query(Read::new())),
