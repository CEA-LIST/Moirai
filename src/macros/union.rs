                            ...
                            $union::$variant(o) => {
                                match &mut self.child {
                                    [<$union Container>]::Unset => {
                                        let log = {
                                            let mut log = $log::new();
                                            let child_op: <$log as IsLog>::Op = <$ty as $crate::utils::boxer::Boxer<_>>::boxer(o);
                                    ...
                                    [<$union Container>]::Value(existing_child) => {
                                        if let [<$union Child>]::$variant(existing_log) = existing_child.as_mut() {
                                            let child_event = $crate::protocol::event::Event::unfold(event, <$ty as $crate::utils::boxer::Boxer<_>>::boxer(o));
